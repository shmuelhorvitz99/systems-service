...
export class FeaturesManager {
    static async getByQuery(query: Partial<Feature>, step: number, limit?: number): Promise<FeatureDocument[]> {
        return FeaturesModel.find(query, {}, limit ? { limit, skip: limit * step } : {}).exec();
    }

    static async getCount(query: Partial<Feature>): Promise<number> {
        return FeaturesModel.count(query);
    }

    static async getById(featureId: string): Promise<FeatureDocument> {
        return FeaturesModel.findById(featureId).orFail(new DocumentNotFoundError(featureId)).exec();
    }

    static async createOne(feature: Feature): Promise<FeatureDocument> {
        return FeaturesModel.create(feature);
    }

    static async updateOne(featureId: string, update: Partial<Feature>): Promise<FeatureDocument> {
        return FeaturesModel.findByIdAndUpdate(featureId, update, { new: true }).orFail(new DocumentNotFoundError(featureId)).exec();
    }

    static async deleteOne(featureId: string): Promise<FeatureDocument> {
